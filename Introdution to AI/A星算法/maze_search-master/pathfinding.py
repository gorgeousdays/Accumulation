...
def manhattan(node, end):
	## Your code goes here. Just 1 line is ok.
...
def chebyshev(node,end):
	return  abs(node[0] - end[0]) + abs(node[1] - end[1])+(math.sqrt(2)-2)*min(abs(node[0] - end[0]),abs(node[1] - end[1]))
